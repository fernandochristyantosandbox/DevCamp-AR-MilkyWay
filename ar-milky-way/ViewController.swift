    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        DispatchQueue.global(qos: .background).async { [weak self] in
            ...
            self.planets.forEach { planet in
                ...
                let planetAxis = SCNNode(geometry: SCNPlane(width: 0.0, height: 0.0))
                planetAxis.position = self.sunPosition
                
                let planetNode = SCNNode(geometry: SCNSphere(radius: planet.diameter))
            
                planetNode.position = self.sunPosition.append(planet.distance)
                ...
                for accessory in planet.planetAccessory {
                    let planetAccessory = accessory.node
//                    planetAccessory.position = planetNode.position.append(accesory.distance)
                    planetAccessory.eulerAngles = accessory.angle
                    planetAccessory.geometry?.firstMaterial?.diffuse.contents = accessory.skin
                    planetNode.addChildNode(planetAccessory)

                }
                
                let planetRotateAction = SCNAction.rotate(by: 2.0, around: SCNVector3(0, 1, 0), duration: planet.rotationSpeed)
                let rotateForever = SCNAction.repeatForever(planetRotateAction)
                planetAxis.runAction(rotateForever)
                
                let orbit = SCNNode(geometry: SCNTorus(ringRadius: CGFloat(planetNode.position.x), pipeRadius: 0.0001))
